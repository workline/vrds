...
public class RepoItem implements Serializable {
	...
	public RepoItemAttribute getAttribute(String attributeDefinitionName) {
		...
		if (attributeDefinitionName == null
			...
			boolean found = false;
			Iterator<RepoItemAttribute> attributesIterator = repoItemAttributes
					.iterator();

			while (!found && attributesIterator.hasNext()) {
				RepoItemAttribute currentAttribute = attributesIterator.next();
				String currentAttributeDefinitionName = currentAttribute
						.getDefinition().getName();

				if (attributeDefinitionName
						.equals(currentAttributeDefinitionName)) {
	...
	public Object getValue(String attributeDefinitionName) {
		Object attributeValue = getAttribute(attributeDefinitionName)
				.getValue();
	...
	public Set<IValue<Object>> getValues(String attributeDefinitionName) {
		Set<IValue<Object>> attributeValues = getAttribute(
				attributeDefinitionName).getValues();
	...
	public String toString() {
		return "RepoItem [id=" + id + ", definitionName="
				+ (definition == null ? "N/A" : definition.getName())
				+ ", repoItemAttributes=" + repoItemAttributes + "]";
	}
